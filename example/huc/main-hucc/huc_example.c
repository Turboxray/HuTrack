...
#asm
		; Move the current .code position to the current .data position
		; because HuTrack wants to load sound data into the .code area!
		.data
sound_data_1st	=	*
		.code
		.bank	bank(sound_data_1st) - _bank_base
		.org	sound_data_1st
#endasm
...
#asm
		; Move the current .data position to the current .code position
		; after HuTrack's sound data has been loaded, and then move the
		; .code position back to HuCC's CORE_BANK.
		.code
		.page	3
sound_data_end	=	*
		.data
		.bank	bank(sound_data_end) - _bank_base
		.org	sound_data_end
		.code
		.bank	CORE_BANK
#endasm
